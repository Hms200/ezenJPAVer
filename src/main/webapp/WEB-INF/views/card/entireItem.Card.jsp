  	...
  	<c:if test="${ entireItemCardMode == 0  }">
		...
		<div class="container-fluid px-0 mt-2 mb-1 col-12 d-flex justify-content-start text-center" style="white-space:nowrap; overflow-x: auto;">
	        ...
	        <div class="col-5 inline-block mx-1 my-2 px-0 border rounded ">
	            <a href="../goodsList/goodsDetail?goods_idx=${ list.goodsIdx }">
	            	<img src="${ list.goodsThumb }" class="card-img-top" alt="goods_idx:${ list.goodsIdx }_img">
		            <div class="card-body py-2 pl-3 pr-3 text-dark text-decoration-none"> 
		            	<h5 class="card-title font-weight-bold font-primary" >${ list.goodsName }</h5>
		            	<p class="card-text font-secondary" >${ list.goodsPrice }원</p>
		            </div>
	            </a>
	        </div>
        	</c:forEach>   
     ...
     <c:if test="${ entireItemCardMode eq 2 }">
	        ...
	        <div class="container-fluid px-0 mt-2 mb-1 col-12 d-flex justify-content-start text-center flex-wrap mx-auto">
		        ...
		        <div class="col-5 inline-block mx-auto my-2 px-0 border rounded position-relative" style="cursor: pointer">
		            <input type="checkbox" class="position-absolute" name="${ list.goodsIdx }" id=${ list.goodsIdx } style="left: 0; top: 0; width: 15px;height: 15px;">
			        <label for="${ list.goodsIdx }">
			         ...
			         <a href="../goodsList/goodsDetail?goods_idx=${ list.goodsIdx }">
			            <div class="card-body py-2 pl-3 pr-3 text-dark text-decoration-none"> 
			            	<h5 class="card-title font-weight-bold font-primary">${ list.goodsName }</h5>
			            	<p class="card-text mb-1 font-secondary">${ list.goodsPrice }원</p>
			            	<p class="card-text">
			            		<small>재고 : ${ list.goodsStock }개
			            		  <c:if test="${ list.goodsOnSale == 1 }">/ 판매중</c:if>
			            		  <c:if test="${ list.goodsOnSale == 0 }">/ 판매중지</c:if><br>
			            		  <c:if test="${ list.goodsOnEvent == 1 }">- 할인상품 -</c:if>
			            		  <c:if test="${ list.goodsOnEvent == 2 }">- 추천상품 -</c:if>
			            		  <c:if test="${ list.goodsOnEvent == 3 }">- 행사상품 -</c:if>
